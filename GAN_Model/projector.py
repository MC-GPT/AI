...
if __name__ == '__main__':
    ...
    face_img = Image.fromarray(img_crop(args.files))
    ...
    ensure_checkpoint_exists('Models/face.pt')
    g_ema.load_state_dict(torch.load('Models/face.pt', map_location='mps')['g_ema'], strict=False)
    ...
    filename = f'{out_dir}/{os.path.splitext(os.path.basename(args.files))[0]}.pt'

    img_ar = make_image(img_gen)

    real_file = str(args.files)
    '''
    for i, input_name in enumerate(real_file):
        result_file['latent'] = latent_in[i]
    '''
    torch.save({'latent': latent_in}, filename)
